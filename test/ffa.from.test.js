...
test('FFAtoFFA', function *(t) {
  ...
  t.eq(f2.matches.length, 2+1, '3 matches f2');
  t.eq(f2.players(), [9,10,11,12,13,14,15,16], 'advancers from f1');
  // and in fact the seeds map on to who won in d1 (top8 below)
  // group(8,4) gives [ [ 1, 3, 6, 8 ], [ 2, 4, 5, 7 ] ]

  t.eq(f2.matches[0].p, [16,14,11,9], '1,3,6,8 m1');
  t.eq(f2.matches[1].p, [15,13,12,10], '2,4,5,7 in m2');
  ...
  t.eq(f2.matches[2].p, [15,16,13,14], '(2,1),(4,3) in m3');

  var top4 = $.pluck('seed', f2.results().slice(0, 4));
  t.eq(top4, [15,16,13,14], 'winners top 4 seeds in f2');

  var f3 = FFA.from(f2, 4);
  t.eq(f3.players(), [13,14,15,16], 'top 4 progressed to f3');
});
