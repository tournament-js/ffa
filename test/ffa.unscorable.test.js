...
test('history', function *(t) {
  ...
  ffa.findMatches({r: 1}).forEach(function (m) {
    t.equal(ffa.unscorable(m.id, [1,2,3,4]), null, 'scorable unplayed r1');
  ...
  t.deepEqual(ffa.players({r:2}), [1,2,3,4,5,6,7,8], 'top half won r1');

  // but not if it has started
  ffa.score({s:1, r:2, m:1}, [1,2,3,4], 'score a match in r2');
  ffa.findMatches({r: 1}).forEach(function (m) {
    t.equal(ffa.unscorable(m.id, [1,2,3,4]),
      m.id + ' cannot be re-scored',
      'unscorable played r1'
    );
  });
  t.deepEqual(ffa.players({r:2}), [1,2,3,4,5,6,7,8], 'top half STILL won r1');

  // but can (re)score (first)rest of r2
  ffa.findMatches({r: 2}).forEach(function (m) {
    t.equal(ffa.unscorable(m.id, [4,3,2,1]), null, 'scorable semi-played r2');
    t.ok(ffa.score(m.id, [4,3,2,1]), 'score semi-played r2')
  });
  t.deepEqual(ffa.players({r:3}), [1,2,3,4], 'top half won r2');

  // score r3 now - can see this works
  var gf = { s: 1, r: 3, m: 1 };
  t.equal(ffa.unscorable(gf, [4,3,2,1]), null, 'scorable unplayed r3');
